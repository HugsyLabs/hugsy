...
export function ConfigEditor() {
  ...
  const [expandedFolders, setExpandedFolders] = useState<Set<string>>(new Set(['git', 'development', 'documentation', 'release']));
  ...
  const settingsString = compiledSettings ? JSON.stringify(compiledSettings, null, 2) : '// Click "Preview" to generate settings.json';
  ...
  const handleCommandDelete = async (file: CommandFile) => {
    ...
    try {
      ...
      addLog({ level: 'info', message: `Command '${commandName}' removed from preview (file may not exist locally)` });
    }
    ...
    if (compiledCommands) {
      ...
      const updatedFolders = commandFolders.map(folder => ({
        ...folder,
        files: folder.files.filter(f => f.path !== file.path)
      ...
      try {
        ...
        const toBeAdded = Array.from(remainingCompiledNames).filter(name => !localCommandNames.has(name));
        const existing = Array.from(remainingCompiledNames).filter(name => localCommandNames.has(name));
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              title={!compiledSettings ? "Preview first to enable install" : "Install settings.json to your project"}
              className={cn(
                    ...
                    <div className="h-[42px] px-3 flex items-center border-b border-gray-200 dark:border-gray-800">
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center space-x-1.5">
                          ...
                          <span className="text-sm font-semibold text-gray-900 dark:text-white">/commands</span>
                        </div>
